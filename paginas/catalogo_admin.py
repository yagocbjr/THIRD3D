...
def render_catalogo_admin(conn, cursor, converter_imagem_base64):
    st.title("🧩 Gerenciar Catálogo de Peças")
    ...
    if not materiais_cadastrados:
        st.warning("⚠️ Cadastre materiais no estoque antes de registrar peças padrão!")
    else:
        with st.expander("➕ Cadastrar Nova Peça Padrão", expanded=True):
            ...
            with col_c1:
                nome_peca_padrao = st.text_input(
                    ...
                    key="cat_nome_peca"  # ✅ KEY ADICIONADA
                )
            ...
            res1.write(f"📊 **Custo de Produção:** R$ {custo_producao_calc:.2f}")
            res2.write(f"💰 **Preço Sugerido de Venda:** R$ {preco_final_padrao:.2f}")

            if st.button("💾 Salvar Peça no Catálogo", type="primary", key="cat_btn_salvar"):  # ✅ KEY ADICIONADA
            ...
            for i in range(0, len(pecas), cols_per_row):
                ...
                for j in range(cols_per_row):
                    if i + j < len(pecas):
                        ...
                        p = pecas[idx]
                        (
                            p_id, p_nome, mc_nome, m_cor, m_tipo,
                            p_peso, p_tempo, p_preco, p_custo, p_imagem,
                        ) = p
